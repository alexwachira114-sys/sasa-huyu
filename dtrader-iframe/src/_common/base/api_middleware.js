...
const { transformV2Request } = require('./deriv_v2_adapter');
...
class APIMiddleware {
    ...
    requestDataTransformer(request) {
        const transformed_request = isV2Api() ? transformV2Request(request) : request;
        return this.session_id ? { ...transformed_request, session_id: this.session_id } : transformed_request;
    }
