...
export const getAppId = () => {
    ...
    // 1. URL param ?app_id=XXXXX takes highest priority — supports iframe embedding from external hosts.
    //    Store it to localStorage so it persists for the entire session without needing it in every URL.
    const url_app_id = new URLSearchParams(window.location.search).get('app_id');
    if (url_app_id && /^\d+$/.test(url_app_id)) {
        window.localStorage.setItem('config.app_id', url_app_id);
        return url_app_id;
    }
...
export const getSocketURL = (is_wallets = false) => {
    ...
    // Resolve active loginid from URL params (iframe embed) or stored session.
    ...
    // ws.derivws.com is the unified endpoint compatible with both legacy and new OAuth API.
    // It routes internally based on the token, so no blue/green switching is needed.
