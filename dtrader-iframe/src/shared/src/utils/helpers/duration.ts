...
export const buildDurationConfig = (
    ...
    const start_type = contract.start_type === 'forward' ? 'forward' : 'spot';
    durations.units_display[start_type] = durations.units_display[start_type] || [];
    ...
    const expiry_type =
        ...
        (obj_min.unit === 't' ? 'tick' : obj_min.unit === 'd' ? 'daily' : 'intraday');
    const duration_min_max = durations.min_max[start_type] as Partial<Record<'tick' | 'intraday' | 'daily', TMaxMin>>;

    duration_min_max[expiry_type as keyof typeof duration_min_max] = {
    ...
    durations?.units_display?.[start_type]?.forEach?.(obj => {
        arr_units.push(obj.value);
    });

    const duration_maps = getDurationMaps();

    if (/^(?:tick|daily)$/.test(expiry_type)) {
    ...
    durations.units_display[start_type] = arr_units
