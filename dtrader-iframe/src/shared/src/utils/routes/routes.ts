...
export const isDtraderRoute = (path = window.location.pathname) =>
    path === routes.trade || path === routes.trader_positions || path.startsWith('/contract/');

export const isDisabledLandscapeBlockerRoute = (path: string) => {
    if (path === routes.trade) return true;
