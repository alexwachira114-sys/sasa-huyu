...
const initStore = (notification_messages, accounts) => {
    ...
    const api_version = url_params.get('api_version');
    if (api_version) {
        setDerivApiVersion(api_version);
    } else if (window.self === window.top) {
        setDerivApiVersion('v1');
    }
    // start timers to measure performance
    startPerformanceMetrics(url_query_string, url_params);

    // Persist app_id from URL so WebSocket uses the caller's registered app.
    const url_app_id = url_params.get('app_id');
    if (url_app_id && /^\d+$/.test(url_app_id)) {
        localStorage.setItem('config.app_id', url_app_id);
    }
