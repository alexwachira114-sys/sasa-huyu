...
import { getBridgeAuth } from './iframe-bridge';
...
const getWebSocketURL = (endpoint: string) => {
    if (isV2Api()) {
        ...
        if (bridge?.otpUrl) return bridge.otpUrl;

        const stored_otp_url = typeof sessionStorage !== 'undefined' ? sessionStorage.getItem('v2_ws_url') : null;
        if (stored_otp_url) return stored_otp_url;
    }
    ...
    if (!connectionRef.current) {
        connectionRef.current = initializeConnection(
            ...
            () => {
                ...
                if (isV2Api()) {
                    wsClientRef.current.setAuthorized();
                }
