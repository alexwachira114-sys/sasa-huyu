...
import { getAccountsFromLocalStorage, getActiveLoginIDFromLocalStorage, getToken, isV2Api } from '@deriv/utils';
...
import { getBridgeAuth, refreshAuth } from './iframe-bridge';
...
const AuthProvider = ({ loginIDKey, children, cookieTimeout, selectDefaultAccount, logout }: AuthProviderProps) => {
    ...
    function initialize() {
        ...
        if (isV2Api()) {
            ...
            const active_account = bridge.accounts.find(account => account.account_id === bridge.activeAccountId);
            const synthetic_authorize_response = {
                authorize: {
                    loginid: bridge.activeAccountId,
                    balance: Number(active_account?.balance ?? 0),
                    currency: active_account?.currency || bridge.userProfile.currency || 'USD',
                    country: bridge.userProfile.country,
                    local_currencies: bridge.userProfile.local_currencies,
                    account_list: bridge.accounts.map(account => ({
                        loginid: account.account_id,
                        currency: account.currency,
                        is_virtual: account.account_type === 'demo' ? 1 : 0,
                    })),
                    email: bridge.userProfile.email,
                    fullname: bridge.userProfile.fullname,
                },
            } as unknown as TSocketResponseData<'authorize'>;

            setIsAuthorized(true);
            processAuthorizeResponse(synthetic_authorize_response);
            setLoginid(bridge.activeAccountId);
            setIsSuccess(true);
            setIsLoading(false);
            setIsInitializing(false);
            return () => {};
    ...
    const switchAccount = useCallback(
        async (newLoginId: string, forceRefresh?: boolean) => {
            if (isV2Api()) {
                // For v2/OAuth, request fresh auth from the bridge (parent will supply new account auth)
                // then re-initialize so the app reflects the new active account.
                if (newLoginId === loginid && !forceRefresh) return;
                try {
                    setIsLoading(true);
                    setIsSwitching(true);
                    const freshAuth = await refreshAuth(8000);
                    const active_account = freshAuth.accounts.find(a => a.account_id === freshAuth.activeAccountId);
                    const synthetic_authorize_response = {
                        authorize: {
                            loginid: freshAuth.activeAccountId,
                            balance: Number(active_account?.balance ?? 0),
                            currency: active_account?.currency || freshAuth.userProfile.currency || 'USD',
                            country: freshAuth.userProfile.country,
                            local_currencies: freshAuth.userProfile.local_currencies,
                            account_list: freshAuth.accounts.map(account => ({
                                loginid: account.account_id,
                                currency: account.currency,
                                is_virtual: account.account_type === 'demo' ? 1 : 0,
                            })),
                            email: freshAuth.userProfile.email,
                            fullname: freshAuth.userProfile.fullname,
                        },
                    } as unknown as TSocketResponseData<'authorize'>;
                    processAuthorizeResponse(synthetic_authorize_response);
                    setIsSuccess(true);
