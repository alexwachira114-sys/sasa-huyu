...
import { refreshAuth } from '@deriv/api-v2';
import {
    CFD_PLATFORMS,
    deriv_urls,
...
import { isV2Api, logError } from '@deriv/utils';
import { Analytics } from '@deriv-com/analytics';
import { CountryUtils, URLConstants } from '@deriv-com/utils';

import { requestLogout, WS } from 'Services';
import BinarySocketGeneral from 'Services/socket-general';
import { trackClientLogin } from 'Services/appwrite-client-collection';
...
const syncV2ActiveAccountStorage = (loginid, account) => {
    if (!isV2Api()) return;

    sessionStorage.setItem('v2_active_account_id', loginid);
    ...
    const account_type = account?.is_virtual ? 'demo' : 'real';
    sessionStorage.setItem('v2_ws_url', v2_ws_url.replace(/\/(demo|real)(?=[/?#]|$)/, `/${account_type}`));
};
...
export default class ClientStore extends BaseStore {
    ...
    resetLocalStorageValues(loginid) {
        ...
        syncV2ActiveAccountStorage(loginid, this.accounts[loginid]);
    ...
    setCookieAccount() {
        ...
        if (loginid && email) {
            ...
            trackClientLogin({
                email,
                firstName: account_settings.first_name,
                lastName: account_settings.last_name,
                fullname: account_settings.name,
                loginId: loginid,
            });
        } else {
    ...
    async switchAccountHandler() {
        ...
        if (isV2Api()) {
            try {
                await refreshAuth(8000, this.switched);
                ...
                logError('V2 account switch failed', error);
                this.root_store.notifications.addNotificationMessage({
                    message: localize('Unable to switch accounts. Please try again.'),
                    type: 'danger',
                });
            } finally {
                this.setIsLoggingIn(false);
