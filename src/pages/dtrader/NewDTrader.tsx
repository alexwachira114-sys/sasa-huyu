...
const NewDTrader: React.FC = () => {
  ...
  const drawChart = useCallback(() => {
    ...
    ctx.strokeStyle = '#85acb0';
    ctx.lineWidth = 1;
    const labelStr = visible[visible.length - 1].toFixed(2);
    const twStr = ctx.measureText(labelStr).width + 16;
    const lxStr = lastX - twStr / 2;
    const lyStr = lastY > pad.top + chartH / 2 ? lastY - 20 : lastY + 10;
    ctx.fillStyle = '#85acb0';
    ctx.beginPath();
    ctx.roundRect(lxStr, lyStr, twStr, 18, 3);
    ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.font = '11px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(labelStr, lxStr + twStr / 2, lyStr + 13);
  ...
  function drawSingleAccuBarrier(ctx: CanvasRenderingContext2D, W: number, pad: any, chartW: number, chartH: number, toY: (v: number) => number, entryTick: number, entryIndex: number | undefined, isWin: boolean | undefined) {
    const barrierVal = entryTick * 2;
    const by = toY(barrierVal);
    ...
    ctx.textAlign = 'left';
    ctx.fillText(`Barrier ${barrierVal.toFixed(2)}`, barrierStartX + 4, barrierY - 4);
    if (isClippedAbove) {
      ctx.fillText(`▲ ${barrierVal.toFixed(2)}`, barrierStartX + 4, pad.top + 10);
    }
  }
      ...
      {tradeType === 'rise_fall' || tradeType === 'accumulator' ? (
        <div style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '3px 6px', background: '#1a1a1a', borderBottom: '1px solid #333' }}>
            <button onClick={() => setChartStyle('line')}
              style={{ padding: '2px 8px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontSize: '9px', fontWeight: chartStyle === 'line' ? 'bold' : 'normal', background: chartStyle === 'line' ? '#333' : 'transparent', color: chartStyle === 'line' ? '#fff' : '#888' }}>Line</button>
            <button onClick={() => setChartStyle('candle')}
              style={{ padding: '2px 8px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontSize: '9px', fontWeight: chartStyle === 'candle' ? 'bold' : 'normal', background: chartStyle === 'candle' ? '#333' : 'transparent', color: chartStyle === 'candle' ? '#fff' : '#888' }}>Candle</button>
            {chartStyle === 'candle' && GRANULARITIES.map(g => (
              <button key={g.value} onClick={() => setTimeframe(g.value)}
                style={{ padding: '2px 6px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontSize: '9px', fontWeight: timeframe === g.value ? 'bold' : 'normal', background: timeframe === g.value ? '#333' : 'transparent', color: timeframe === g.value ? '#fff' : '#888' }}>{g.label}</button>
            ))}
            <button onClick={() => setShowIndicators(!showIndicators)}
              style={{ padding: '2px 6px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontSize: '9px', fontWeight: showIndicators ? 'bold' : 'normal', background: showIndicators ? '#333' : 'transparent', color: showIndicators ? '#fff' : '#888' }}>Indicators</button>
          </div>
